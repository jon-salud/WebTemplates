...
const Benefits: React.FC<BenefitsProps> = ({
  ...
  const theme = {
    ...
    cardHover: isDarkMode ? 'hover:border-white/20' : 'hover:border-transparent',
    ...
    <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8">
      {industry.benefits.items.map((benefit, index) => (
        ...
        >
          ...
          <h3 className={`text-heading-3 ${theme.text} mb-2`}>
            {benefit.title}
          </h3>
          <p className={`text-body-sm ${theme.textMuted}`}>
            {benefit.description}
          </p>
        </motion.div>
      ))}
